	...
	def size
	puts "Il y à " "#{@journalist.size}" " journalistes dans la liste."
	end
	

	def number
	number = @journalist.select {|e| e =~ /[[:digit:]]/}
	puts "#{number.size}" " ont un numéro dans leur pseudonyme"
	end


	def aude
	aude = @journalist.select {|e| e =~ /aude/i}
	puts "#{aude.size}" " journalistes s'apellent Aude ."
	end


	def majstart
	majstart = @journalist.select {|e| e[1] == e[1].upcase && e[1] != "_" }
	puts "#{majstart.size}" " journalistes on un nom qui commence par une majuscule "
	end


	def majuscule
	majuscule = @journalist.select {|e| e =~ /[[A-Z]]/ } 
	puts "#{majuscule.size}" " journalistes ont une majuscule dans leur nom ."
	end
